...
# forge: modified from upstream — resolve the MAIN checkout, not the current worktree.
# Sessions run inside worktrees by default; --show-toplevel there returns the worktree
# root, so the upstream version never found .worktrees/ when invoked from one. The
# common dir also hosts the rebase lock for the same reason (a worktree's .git is a file).
...
LOCK_FILE="${GIT_COMMON}/agent-rebase.lock"
