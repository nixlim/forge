...
These principles govern the adversarial review process for {{FORGE_PROJECT_NAME}}. The reviewer MUST evaluate the spec or code against every applicable principle. When a principle is violated, it becomes a finding.
...
**Profile set version: 1.0** ({{FORGE_INSTALL_DATE}}). Changes to the baseline or any profile
...
<!-- forge-init: replace with 2–4 project-specific code-review completeness items — the recurring "did you check X" gates for this stack (resource-leak / lifecycle balance, module or dependency-boundary conformance, generated-artifact or consumer-doc sync, etc.). -->
- [ ] _(No project-specific completeness items configured yet — `/forge-init` populates this region.)_
...
<!-- forge-init: seed this trigger table from the project's stack, bug history, and known failure modes. Add one row per high-risk pattern (Pattern | Required Checks). For each trigger you add, also add the corresponding project principle(s) into the lens tables above — give them IDs that continue each lens's numbering (e.g. the next AMB-*, INC-*, SEC-* …) and mark each "Project-specific:". -->

| Pattern | Required Checks |
|---------|----------------|
| `<high-risk pattern for this project>` _(placeholder)_ | `<what a reviewer MUST verify whenever this pattern appears in code or specs>` |
