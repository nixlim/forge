...
name: pr-review
description: >
  Reviews all open pull requests on the project's GitHub repository and produces
...
Analyze all open pull requests on the project's GitHub repository and produce
...
coordinates wherever a `<...>` placeholder appears:

| Placeholder | Meaning |
|-------------|---------|
| `<OWNER>/<REPO>` | GitHub owner/repo for the project |
| `<API_BASE>` | REST API base, e.g. `https://api.github.com/repos/<OWNER>/<REPO>` |

<!-- forge-init: Fill this table with the project's real coordinates (or inline
the concrete values into the commands below). If the project has bot authors
(dependency-update bots, security-scanner bots) or dependency-manifest files
that this skill should special-case for duplicate detection, list them here. Keep
the classification rules, duplicate-detection signals, and report format as they
are. -->
...
curl -sH "Authorization: token $GITHUB_TOKEN" \
  ...
  "<API_BASE>/pulls?state=open&per_page=100&page=1" \
  | jq '[.[] | {
    number,
    title,
    user: .user.login,
...
curl -sH "Authorization: token $GITHUB_TOKEN" \
  -H "Accept: application/vnd.github.v3+json" \
  "<API_BASE>/compare/{base_ref}...{head_sha}" \
...
- Has at least one approving review, OR is from a bot (dependency-update or security-scanner bots)
- Not a draft
...
2. **Similar titles:** PRs with similar titles (e.g., both bump the same dependency)
3. **Same author + same dependency:** Multiple bot PRs for the same package
...
```markdown
# {{FORGE_PROJECT_NAME}} PR Review Report
...
- Bot PRs (dependency-update, security-scanner) often accumulate and become stale; prioritize identifying these
- PRs that only modify dependency manifests or lock files for version bumps are often duplicated by multiple bots
