...
description: >
  ...
  feasibility, security, and project-specific concerns. Loaded by
...
Project-specific spec-review focus is defined in the project context region
below. Apply those principles with the same priority as the generic ones above.

<!-- forge: removed upstream MockServer-specific Lens Priority bullets (Netty ByteBuf lifecycle INC-13, ring buffer sizing analysis INC-14, javax/jakarta compatibility FEA-06); relocated to the skill-project-context region -->

### Project-Specific Spec Focus

<!-- FORGE:REGION skill-project-context BEGIN -->

When the spec touches these project-specific areas, apply the corresponding
completeness and feasibility checks in addition to the generic lenses above.

| Area | Required Checks |
|------|----------------|
| _(none configured yet)_ | _(none configured yet)_ |

<!-- forge-init: Replace this region with the project's spec-review focus areas.
Follow the shape of the upstream table — one `Area | Required Checks` row per
project-specific concern, e.g.: resource-lifecycle specification (allocate/free
semantics); data-structure sizing / capacity analysis; dependency version or
language/runtime compatibility policy; consumer-doc and client-library mirroring;
control-plane vs data-plane boundaries. Cite the project's architecture docs so
reviewers can verify claims about existing behaviour. -->
