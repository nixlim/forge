...
The commands below use readable placeholders. Substitute the project's actual
coordinates and toolchain wherever a `<...>` placeholder appears:

| Placeholder | Meaning |
|-------------|---------|
| `<OWNER>/<REPO>` | GitHub owner/repo for the project (used by every `gh`/`curl` call) |
| `<ISSUE_URL>` | Issue URL pattern, e.g. `https://github.com/<OWNER>/<REPO>/issues/{number}` |
| `<VERSION_SOURCE>` | Where the in-development project version is declared (build/manifest file) |
| `<TEST_CMD>` | Command that runs tests for a given module/package |
| `<CONFIG_SRC>` | Source path for configuration/property definitions |
| `<CONSUMER_DOCS>` | Path to consumer/user-facing documentation |
| `<ARCH_DOCS>` | Architecture / module-overview doc(s) describing module boundaries |

<!-- forge-init: Fill this table with the project's real coordinates and
toolchain, or inline the concrete values directly into the commands below. Also
list the project-specific user-error patterns, deployment forms, and test-
framework conventions that this skill should look for (upstream examples:
mismatched content types, matching-semantics confusion, transport mismatch,
build-plugin/classpath issues, container networking). Keep the classification
tables, commit workflow, and closing-comment templates as they are. -->
...
**URL Pattern:** `<ISSUE_URL>`

Fetch the full issue with comments:

```bash
gh issue view {number} --repo <OWNER>/<REPO> --json number,title,body,author,createdAt,updatedAt,state,labels,comments,assignees
```
...
- **Environment**: product version, runtime/language version, OS, deployment method
...
### 2a: Check Product Version

```bash
# Current released version
gh release list --repo <OWNER>/<REPO> --limit 1 --json tagName,publishedAt
```

```bash
# In-development version (from the project's version source — target the project
# version, not a parent/plugin/transitive version)
# See <VERSION_SOURCE> in the Project Context region.
```
...
```bash
ISSUE_DATE=$(gh issue view {number} --repo <OWNER>/<REPO> --json createdAt --jq '.createdAt[:10]')
```
...
```bash
# Search closed PRs
gh pr list --repo <OWNER>/<REPO> --state closed --search "{keywords from issue title}" --json number,title,mergedAt --limit 10
```
...
grep -rl "{sanitised_class_name_or_keyword}" .
```
...
grep -rl "{property_name}" <CONFIG_SRC>
```

```bash
# Check documentation for the feature
grep -rl "{feature_or_property}" <CONSUMER_DOCS>
```

Common user error patterns (see the Project Context region for the project's
specific patterns):
- Incorrect expectation/request configuration syntax
- Wrong or mismatched content-type headers
- Misunderstanding of matching semantics (substring vs exact, etc.)
- Using a feature with the wrong transport/protocol
- Port conflicts or incorrect server lifecycle management
- Build-plugin or classpath issues in the integration layer
- Container networking issues (localhost vs container networking)
...
```bash
# Run a specific test (see <TEST_CMD> in the Project Context region)
<TEST_CMD> {module} {TestClass}#{testMethod}
```
...
Example pattern (adapt to the project's language and error idioms):
```
...
1. Identify the relevant documentation page under `<CONSUMER_DOCS>`
...
Feature requests require careful evaluation. {{FORGE_PROJECT_NAME}}'s value comes from a simple, predictable API. Every new feature adds surface area that users must learn, maintainers must support, and that can interact unexpectedly with existing features.

### 5a: Check if the Requested Behaviour Already Exists

Before evaluating a new feature, check whether {{FORGE_PROJECT_NAME}} can already do what the user wants:

```bash
# Search documentation for the requested capability
grep -rl "{feature_keyword}" <CONSUMER_DOCS>
```

```bash
# Search the codebase for existing support
grep -rl "{feature_keyword}" <CONFIG_SRC>
```

Common cases where the feature already exists:
- The user wants a specific behaviour that is already supported via a different mechanism
...
- The user wants behaviour achievable through composition of existing features
...
Follow the Fix Placement Policy from AGENTS.md: fix the bug at the architecturally correct layer. If the bug surfaces in one module but the root cause is in a lower-level module, fix it in the lower-level module.

Read the project's architecture / module overview (`<ARCH_DOCS>`) to understand module boundaries.
...
3. Follow existing test conventions in the module (match the framework and style of neighbouring tests)

### 6d: Run Tests

```bash
# Run tests for affected modules (see <TEST_CMD> in the Project Context region)
<TEST_CMD> {affected_modules}
```
...
Run ALL validations for ALL categories of changed files (tests, doc link checks, etc.).
...
```bash
gh issue close {number} --repo <OWNER>/<REPO> --comment "$(cat <<'EOF'
This issue has been resolved. The fix was included in commit {sha} ({brief description}).

If you are still experiencing this issue, please ensure you are using {{FORGE_PROJECT_NAME}} version {version} or later. If the problem persists on the latest version, please open a new issue with updated reproduction steps.
EOF
)"
```
...
```bash
gh issue close {number} --repo <OWNER>/<REPO> --comment "$(cat <<'EOF'
Thank you for the feature request. {{FORGE_PROJECT_NAME}} already supports this — {brief explanation of how to achieve it}.
...
**Reason:** {explanation — e.g., "This would add a second way to achieve {X}, which is already possible via {existing approach}. Adding another mechanism would increase the API surface area without new capability, making the project harder to learn and maintain."}

{If applicable: "The existing approach to achieve this is: {description}. See: {link}"}

We try to keep {{FORGE_PROJECT_NAME}}'s API surface minimal and predictable. Every new feature must justify the complexity it adds for all users, not just the requesting use case.
...
- If reproduction requires infrastructure (containers, specific OS, etc.) that isn't available, note this limitation and make a best-effort assessment from code review
