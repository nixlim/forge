---
name: debugger
description: Production debugger — investigates issues using logs, CI data, metrics, and infrastructure. Invoke this skill to diagnose build failures, runtime errors, or infrastructure problems. It reports findings; it does not fix.
---

<!-- forge role skill — generated from .claude/agents/debugger.md (body verbatim).
Kimi Code CLI has no custom sub-agent registry (built-ins: coder / explore / plan), so
forge's DVRR roles ship as project skills: invoke as /skill:debugger, or dispatch a
